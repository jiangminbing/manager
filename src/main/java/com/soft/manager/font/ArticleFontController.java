...
public class ArticleFontController extends BaseFontContrller {
    ...
    @ResponseBody
    public PageResult<Article> getArticleByPage(@RequestBody ArticleSerachDto dto){
        try {
            printParam("getArticleByPage==>"+ JSON.toJSONString(dto));
            PageResult<Article> result = articleService.getArticleByPage(dto);
            return  result;
        }catch (Exception e){
    ...
    public Result getArticleById(@RequestParam Integer id) {
        try {
            ...
            Article articleDto = articleService.getArticleById(id);
            DetailResult<Article> result = new DetailResult<>(ResCode.SUCCESS);
